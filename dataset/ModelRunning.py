...
# >> 네이버 영화 리뷰 중 영화당 100개의 릐뷰를 모아
# >> 총 200,000개의 리뷰(훈련: 15만개, 테스트: 5만개)로
# >> 이루어져 있고, 1~10점 까지의 평점 중 종합적인 평점(5~8)은
...
# 데이터를 합습하기에 알맞게 처리해보자. konlpy 라이브러리를 사용해서
...
# 모델 층을 구성 입력층 출력층을 제외한 층의 갯수가 2개 이상인것을 deep lurnning 이라고 하고 지금처럼 층이1개뿐인것을 shallow lurnning이라한다.
